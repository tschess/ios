...
class Hopper {
    ...
    func allClear(indexSquare: Piece?) -> Bool {
        if(indexSquare == nil){
            return true
        }
        if(indexSquare!.name == "PieceAnte"){
            return true
        }
        return false
    }
    
    func direction_UP(present: [Int], proposed: [Int], state: [[Piece?]], affiliation: String) -> Bool {
        ...
        if(!allClear(indexSquare: state[proposed[0]][proposed[1]])){
            return false
        }
        ...
        for index in stride(from: proposed0, through: present_UP, by: 1) {
            let indexSquare = state[index][present[1]]
            if(!allClear(indexSquare: indexSquare)){ //you need to jump here...
                ...
                let indexSquareNxt = state[indexSquareAdjacent_Nxt_index][present[1]]
        ...
        //gamestate.dict[proposed] = collection //TODO... this bitch...
        return true
    }
    
    func direction_DOWN(present: [Int], proposed: [Int], state: [[Piece?]], affiliation: String) -> Bool {
        var collection: Array<[Int]> = Array<[Int]>()
        if(present[1] - proposed[1] != 0){
            return false
        }
    ...
    func direction_LEFT(present: [Int], proposed: [Int], state: [[Piece?]], affiliation: String) -> Bool {
        var collection: Array<[Int]> = Array<[Int]>()
        ...
        for index in stride(from: proposed1, through: present_UP, by: 1) {
            let indexSquare = state[present[0]][index]
            if(!allClear(indexSquare: indexSquare)){ //you need to jump here...
                ...
                let indexSquareNxt = state[present[0]][indexSquareAdjacent_Nxt_index]
                if(!allClear(indexSquare: indexSquareNxt)){
                    return false
                }
                if(squaresGlide > 1){
    ...
    func direction_RIGHT(present: [Int], proposed: [Int], state: [[Piece?]], affiliation: String) -> Bool {
        var collection: Array<[Int]> = Array<[Int]>()
        if(present[0] - proposed[0] != 0){
            return false
        }
